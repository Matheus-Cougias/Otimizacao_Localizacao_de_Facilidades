...
"""Ao realizar o teste com uma quantidade de 300 clientes, obteve-se os seguintes resultados, onde cada rodada
    ...
    -> Rodada 1: 
        AMPL = 10573249
        Gulosa = 12616067 (GAP = 16.4832%)
        LK = 12107394 (GAP = 14.5096%) 

    -> Rodada 2: 
        AMPL = 10634364
        Gulosa = 12750789 (GAP = 19.9017%)
        LK = 12358462 (GAP = 16.2125%)

    -> Rodada 3: 
        AMPL = 10834304
        Gulosa = 13069716 (GAP = 20.6327%)
        LK = 12562997 (GAP = 15.9557%)"""
...
def main():
    ...
    print('Para o problema normal, o AMPL achou:')
    sol = call_ampl_normal(dat)
    ...
    sol = greedy_heur_normal(dat)
    print('\n')
    print('O resultado pela heurística de Lin Kernighan foi:')
    lin_kernighan_normal(dat, sol)


"""############################### LIN KERNIGHAN ############################"""
""" A partir da solução inicial dada pela heuristica gulosa, o conjunto de facilidades
    de alto nível selecionadas são testadas, trocando estas facilidades pelas facilidades
    não seleciondas, onde, quando o custo total de atendimento para o alto nível é
    otimizado, a solução é atualizada. Com a melhor solução para o alto nível encontrada,
    é possível procurar as de baixo nível de modo similar ao utilizado na heurística
    gulosa."""
def lin_kernighan_normal(dat, sol):
    ni = dat['ni']
    nj = dat['nj']
    p = dat['p']
    q = dat['q']
